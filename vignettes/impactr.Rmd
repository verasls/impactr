...
```{r read_example_data}
acc_data <- read_acc(impactr_example("hip-imu.csv"))
```
...
As with `filter_acc()`, `find_peaks()` default values of the minimun height (`min_height`) and distance (`min_dist`) of the peaks are set to replicate the values used in the calibration study. You should only change them if you have a strong reason to.
